...
export function validatePublicUrl(input: string): URL {
  ...
  // Reject IP literals as well as local/special-use names. DNS is checked at connection time by the transport.
...
export function publicResponseHeaders(headers: Headers): Record<string, string> {
  const result: Record<string, string> = {};
  for (const [key, value] of headers) if (RESPONSE_HEADERS.has(key.toLowerCase())) result[key.toLowerCase()] = value;
  return result;
}

export async function readPublicBody(response: Response, signal: AbortSignal): Promise<Uint8Array> {
