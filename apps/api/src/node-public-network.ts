...
import { isPublicAddress, validatePublicUrl } from './public-network-policy';

/** Node/Bun runtime driver. Validated DNS addresses are handed directly to the TLS connection. */
...
export function createPublicLookup(resolve: ResolveAll = lookup): LookupFunction { return (hostname, options, callback) => {
  resolve(hostname, { all: true, verbatim: true }, (error, addresses) => {
    if (error) { callback(error, '', 0); return; }
    if (!addresses.length || addresses.some(entry => !isPublicAddress(entry.address))) {
      callback(new Error('The destination does not resolve exclusively to public addresses.'), '', 0); return;
    }
...
export async function nodePublicFetch(input: string, init: RequestInit = {}): Promise<Response> {
  const url = validatePublicUrl(input);
  init.signal?.throwIfAborted();
  return new Promise((resolve, reject) => {
    const headers = Object.fromEntries(new Headers(init.headers));
    const req = request(url, {
      method: init.method ?? 'GET', headers: { ...headers, 'accept-encoding': 'identity' },
      lookup: publicLookup, agent: false, signal: init.signal ?? undefined,
    }, incoming => {
      ...
      const body = new ReadableStream<Uint8Array>({
        async pull(controller) {
          try {
            init.signal?.throwIfAborted(); const chunk = await iterator.next(); init.signal?.throwIfAborted();
            if (chunk.done) { cleanUp(); controller.close(); } else controller.enqueue(new Uint8Array(chunk.value));
          } catch (error) { cleanUp(); controller.error(error); abort(); }
        },
